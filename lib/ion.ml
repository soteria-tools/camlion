...
type ion_value = { annotations : string list; value : ion_data }
...
and ion_data =
  ...
  | SymbolId of int
      (** Unquoted symbol ID reference, e.g. [$99] — must be resolved against
          symbol table *)
  | String of string
...
let symbol_needs_quoting s =
  if String.length s = 0 then true
    ...
    in
    let ok_rest c = ok_start c || (c >= '0' && c <= '9') in
    (not (ok_start s.[0]))
    ||
...
and pp_data buf = function
  ...
  | Int n -> Buffer.add_string buf (Z.to_string n)
  | Float f ->
  ...
  | Decimal s -> Buffer.add_string buf s
  | Timestamp s -> Buffer.add_string buf s
  | Symbol s -> pp_symbol buf s
  | SymbolId n ->
      Buffer.add_char buf '$';
