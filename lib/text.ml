...
(** If [s] matches [$ion_N_M], return [(major, minor)] as strings. Uses simple
    prefix/character scanning — no regex dependency. *)
let parse_ion_version s =
  let n = String.length s in
  if n > 6 && String.sub s 0 5 = "$ion_" then
    (* Find the '_' separating major and minor version *)
    begin match String.rindex_opt s '_' with
    | None -> None
    | Some sep ->
...
let parse_symbol_id s =
  ...
  if n >= 2 && s.[0] = '$' then begin
    ...
    then try Some (int_of_string rest) with _ -> None
    else None
  end
  else None
...
let validate_import_struct fields =
  List.iter
    (fun (fname, fval) ->
      if fname = "max_id" then
        begin match fval.value with
        | Int n ->
            if Z.compare n Z.zero < 0 then
              raise
                (Text_lexer.Lexer_error
                      ...
                      List.iter
                        (fun (iname, ival) ->
                          if iname = "max_id" then
                            ...
                            | Int n ->
                                from_imports := !from_imports + Z.to_int n
                            | _ -> ())
                        ifields
                  | _ -> ())
                imports
...
(** Check that all $N symbol IDs in a value are within range. [max_sid] is the
  ...
  List.iter
    (fun ann ->
      ...
      | Some n when n >= max_sid ->
          raise
            (Text_lexer.Lexer_error
               (Printf.sprintf "Symbol ID $%d is out of range (max $%d)" n
                  (max_sid - 1)))
      | _ -> ())
    v.annotations;
  (* Check datum *)
...
let check_symbol_id_ranges values =
  ...
  List.iter
    (fun v ->
      ...
      else check_symbol_ids_in_value !max_sid v)
    values
