...
(** Resolve SID. SID 0 is valid but has no text → returns None (caller decides
    what to do). Out-of-range SIDs raise Failure. *)
let lookup_sym_opt (st : string array) sid =
  if sid < 0 || sid >= Array.length st then
    Parseff.fail
      (Printf.sprintf "symbol ID $%d out of range (table has %d entries)" sid
         (Array.length st))
  else
    let s = st.(sid) in
    if s = "" then None (* SID 0 or explicitly unmapped slot *) else Some s
...
let lookup_sym (st : string array) sid =
  ...
  | Some s -> s
  | None -> Parseff.fail (Printf.sprintf "symbol ID $%d is unmapped" sid)
...
let read_var_uint () =
  ...
  while !go do
    ...
    else v := (!v lsl 7) lor b
  done;
  !v

(** VarInt: first byte bit7=last, bit6=sign, bits5-0=value; subsequent bytes
    same as VarUInt. *)
let read_var_int () =
...
(** Parse Ion binary decimal encoding from bytes. Format: VarInt exponent +
    sign-magnitude coefficient. Returns a string like "3d-2" or "-0d0". *)
let format_decimal_bytes s =
  if String.length s = 0 then "0d0"
    ...
    if b0 land 0x80 = 0 then begin
      ...
      while !go do
        if !pos >= len then
          Parseff.fail "decimal: exponent extends past content";
    ...
    let neg_c =
      String.length coeff_s > 0 && Char.code coeff_s.[0] land 0x80 <> 0
    in
...
let format_timestamp_bytes s =
  let len = String.length s in
  if len < 2 then
    Parseff.fail (Printf.sprintf "timestamp: too short (len=%d, need ≥2)" len);
  let pos = ref 0 in
  ...
  if !pos >= len then Buffer.add_string buf (Printf.sprintf "%04dT" year)
  else begin
    let month = var_uint_s () in
    if month < 1 || month > 12 then
      Parseff.fail (Printf.sprintf "timestamp: invalid month %d" month);
    if !pos >= len then
      Buffer.add_string buf (Printf.sprintf "%04d-%02dT" year month)
    else begin
      let day = var_uint_s () in
      let max_day =
        if month = 2 && is_leap_year year then 29 else days_in_month.(month)
      in
      if day < 1 || day > max_day then
        Parseff.fail
          (Printf.sprintf "timestamp: invalid day %d for month %d year %d" day
             month year);
      if !pos >= len then
        Buffer.add_string buf (Printf.sprintf "%04d-%02d-%02dT" year month day)
      else begin
        ...
        if !pos < len then begin
          ...
          if second > 59 then
            Parseff.fail (Printf.sprintf "timestamp: invalid second %d" second);
            ...
            let frac_neg, mag =
              if String.length mag_bytes = 0 then (false, Z.zero)
                ...
                for i = 1 to String.length mag_bytes - 1 do
                  r :=
                    Z.add (Z.shift_left !r 8)
                      (Z.of_int (Char.code mag_bytes.[i]))
                done;
                (neg, !r)
              end
            in
        ...
        (* Offset suffix *)
        if unknown_offset then Buffer.add_string buf "-00:00"
        else if offset = 0 then Buffer.add_char buf 'Z'
        else begin
...
(** Parse a sequence of VarUInt SIDs from a byte string. Returns list of
    resolved symbol strings. SID 0 in annotation = error. *)
let parse_annotation_list (st : string array) bytes =
  ...
  while !pos < len do
    ...
    let sym =
      match lookup_sym_opt st !v with
      | Some s -> s
      | None ->
...
let process_lst (sym_ref : string array ref)
  ...
  let base =
    ...
    | None -> Array.copy system_symbols
    | Some { value = Symbol "$ion_symbol_table"; _ } -> Array.copy !sym_ref
        ...
        List.iter
          (fun import ->
            ...
            | Struct ifields -> (
                ...
                List.iter
                  (fun (k, v) -> if k = "max_id" then max_id_opt := Some v)
                  ifields;
                ...
                | Some { value = Int n; _ } ->
                    let n = Z.to_int n in
                    if n < 0 then
                      Parseff.fail "import max_id must be non-negative";
                    for _ = 1 to n do
                      slots := !slots @ [ "" ]
                    done
          ...
          imports;
        Array.of_list !slots
    | Some _ -> Array.copy system_symbols
  in
  let extra =
    ...
    | Some { value = List syms; _ } ->
        Array.of_list
          (List.map
             (fun sv -> match sv.value with String s -> s | _ -> "")
             syms)
    | Some _ -> [||]
  in
...
(** Read the length field for a type descriptor with nibble [l]. Handles L=14
    (VarUInt follows) and L=15 (null sentinel = -1). *)
let decode_len l =
  if l = 0xF then -1 (* null *) else if l = 0xE then read_var_uint () else l

(** Parse all values from the current position up to [end_pos]. Returns the list
    of parsed ion_values (NOP pads discarded). This is used for list and sexp
    content. *)
...
and parse_struct sym_ref end_pos =
  ...
  while Parseff.position () < end_pos do
    ...
    if sid = 0 then begin
      ...
      let fname =
        match lookup_sym_opt !sym_ref sid with
        | Some s -> s
...
(** Parse a single typed value given its type nibble [typ] and length nibble
    [l]. Returns [None] for NOP pads. *)
and parse_one sym_ref typ l =
  match typ with
  (* 0x0 – NOP pad (L≠15) or null.null (L=15) *)
  | 0x0 ->
      if l = 0xF then Some { annotations = []; value = Null Null_type }
      else begin
  ...
  (* 0x1 – bool *)
  | 0x1 ->
      if l = 0xF then Some { annotations = []; value = Null Bool_type }
      else if l > 1 then
        Parseff.fail
          (Printf.sprintf "invalid bool length %d (must be 0 or 1)" l)
      else Some { annotations = []; value = Bool (l = 1) }
  (* 0x2 – positive int *)
  | 0x2 ->
  ...
  | 0x7 ->
      ...
      if len < 0 then Some { annotations = []; value = Null Symbol_type }
        ...
        else Some { annotations = []; value = Symbol !sym_ref.(sid) }
      end
  (* 0x8 – string *)
  | 0x8 ->
      let len = decode_len l in
  ...
  (* 0xD – struct *)
  | 0xD ->
      if l = 0xF then Some { annotations = []; value = Null Struct_type }
      else begin
        let end_pos, len_ok =
          if l = 0x1 then begin
            ...
            if vlen = 0 then
              Parseff.fail
                "ordered struct (L=1) with length=0 must have at least one \
                 field";
        ...
        in
        if not len_ok then Some { annotations = []; value = Null Struct_type }
        else begin
  ...
  | 0xE ->
      ...
      if vtyp = 0xE then Parseff.fail "nested annotation wrapper is not valid";
  ...
  | 0xF -> Parseff.fail "type code 0xF is reserved and not valid"
  | _ -> assert false
...
(** Parse the Ion binary stream, starting after the initial BVM has been
    consumed. Updates [sym_ref] as LSTs are encountered. *)
  ...
  while !go do
    ...
    | Some b ->
        ...
        if b = 0xE0 then begin
          ...
          begin match parse_one sym_ref typ l with
          | None -> ()
          | Some v ->
              if List.mem "$ion_symbol_table" v.annotations then
  ...
  let result =
    Parseff.parse_source source (fun () ->
        ...
        if not (b0 = 0xE0 && b1 = 0x01 && b2 = 0x00 && b3 = 0xEA) then
          Parseff.fail
            (Printf.sprintf "binary Ion: invalid IVM %02X %02X %02X %02X" b0 b1
               b2 b3);
        parse_stream sym_ref)
  in
  match result with
  | Parseff.Ok values -> values
  | Parseff.Error { error = `Failure msg; _ } -> failwith ("binary Ion: " ^ msg)
...
let of_string s = run_source (Parseff.Source.of_string s)
let of_channel ic = run_source (Parseff.Source.of_channel ic)
