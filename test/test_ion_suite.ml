(** Run the amazon-ion/ion-tests conformance suite (text + binary formats). Good
    files must parse without exception; bad files must raise. *)
...
let is_ion_binary name = Filename.check_suffix name ".10n"
...
let good_test path =
  ...
  Alcotest.test_case name `Quick (fun () ->
      ...
      | exception exn ->
          close_in_noerr ic;
          Alcotest.failf "Expected parse success but got: %s"
...
let () =
  ...
  let good_files =
    collect_files is_ion_text good_dir @ collect_files is_ion_binary good_dir
  in
  let bad_files =
    collect_files is_ion_text bad_dir @ collect_files is_ion_binary bad_dir
  in
